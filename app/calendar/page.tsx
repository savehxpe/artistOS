      ...
      <main className="max-w-[1400px] mx-auto px-6 pt-32 pb-32 min-h-screen animate-in fade-in duration-1000 slide-in-from-bottom-5">
        ...
        {selectedEvent && (
          <section className="mb-12 group">
            <div className="relative aspect-video bg-zinc-950/50 overflow-hidden border border-white/5 shadow-2xl transition-all duration-700 hover:border-white/10 rounded-3xl">
              <img 
              ...
              />
              <div className="absolute inset-x-4 bottom-4 bg-zinc-950/80 backdrop-blur-xl p-8 text-white border border-white/5 shadow-2xl transition-all rounded-3xl">
                <div className="flex justify-between items-start mb-4">
                  <span className="bg-white text-black px-3 py-1 text-[9px] font-bold tracking-wider rounded-full capitalize transition-all">{selectedEvent?.status?.toLowerCase()}</span>
            ...
            {selectedEvent?.caption && (
              <div className="mt-8 p-10 bg-zinc-950 border border-white/10 relative italic text-[11px] font-inter text-white/50 group shadow-xl rounded-3xl">
                 <div className="absolute -top-4 -left-4 bg-white text-black w-10 h-10 flex items-center justify-center shadow-lg group-hover:rotate-12 transition-transform rounded-xl">
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-4">
            {events.length === 0 ? (
              <div className="py-24 text-center border border-dashed border-white/5 bg-zinc-950/20 rounded-3xl backdrop-blur-sm">
              ...
              events.map((event) => (
                <div 
                  ...
                  className={`group p-8 bento-item transition-all cursor-pointer flex items-center justify-between rounded-3xl ${selectedEvent?.id === event.id ? "bg-white text-black border-white shadow-xl scale-[1.01]" : "bg-zinc-950/30 border-white/5 hover:border-white/10 shadow-sm"}`}
                >
                  <div className="flex items-center gap-8">
                    <div className={`w-16 h-16 flex flex-col items-center justify-center font-manrope font-bold transition-all border rounded-2xl overflow-hidden ${selectedEvent?.id === event.id ? "bg-black text-white border-black" : "bg-zinc-900/50 border-white/5 text-zinc-400 group-hover:bg-zinc-800"}`}>
                      <span className="text-2xl leading-none">{event.date.split('-')[2]}</span>
                      <span className="text-[10px] font-semibold tracking-wider capitalize">{new Date(event.date).toLocaleString('default', { month: 'short' }).toLowerCase()}</span>
                    </div>
                    <div>
                      ...
                      <div className="flex gap-4 mt-2">
                        ...
                        <div className="flex gap-2">
                          {event.platforms?.map(p => (
                            <span key={p} className={`text-[9px] font-semibold tracking-wider px-2 py-0.5 border rounded-full transition-colors ${selectedEvent?.id === event.id ? "border-black/5 text-black/30" : "border-white/5 text-zinc-500"}`}>{p?.toLowerCase()}</span>
                          ))}
                        </div>
                      </div>
                    </div>
        ...
        <button 
          ...
          className="fixed bottom-24 right-6 w-16 h-16 bg-white text-black flex items-center justify-center rounded-full shadow-2xl hover:scale-110 active:scale-90 transition-all z-40 group border border-black/10"
        >
        ...
        {showAddModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/95 backdrop-blur-2xl animate-in fade-in duration-500">
            <form 
              ...
              className="w-full max-w-xl bg-zinc-950 border border-white/10 p-12 md:p-16 space-y-12 shadow-2xl animate-in zoom-in-95 duration-500 relative overflow-hidden text-white rounded-3xl"
            >
              ...
              <div className="space-y-10">
                ...
                <div className="grid grid-cols-2 gap-12">
                  <div className="space-y-4">
                    ...
                    <input 
                      type="date"
                      className="w-full bg-zinc-900/50 border border-white/5 p-6 text-[11px] font-semibold tracking-wider focus:outline-none focus:border-white/20 text-white rounded-xl transition-colors"
                      value={newEvent.date}
                ...
                <div className="space-y-4">
                  ...
                  <textarea 
                    ...
                    className="w-full bg-zinc-900/50 border border-white/5 p-6 text-xs font-inter focus:outline-none focus:border-white/20 h-32 resize-none leading-relaxed placeholder:text-zinc-800 text-white rounded-xl transition-colors"
              ...
              <div className="space-y-4">
                <button 
                  type="submit"
                  className="w-full bg-white text-black py-8 font-semibold tracking-widest text-[11px] hover:bg-zinc-200 transition-all disabled:opacity-30 active:scale-[0.98] rounded-full shadow-lg border border-black/10 uppercase"
