...
function Dashboard() {
  ...
  if (loading || !user) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-black font-inter">
        <div className="w-8 h-8 border border-white/10 border-t-white animate-spin" />
      </div>
    );
  }
    ...
    <div className="bg-black text-white min-h-screen font-inter selection:bg-white selection:text-black">
      <main className="max-w-[1200px] mx-auto px-6 pt-24 pb-32 animate-in fade-in duration-1000">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 auto-rows-min">
          ...
          {/* Phase & Progress */}
          <div className="md:col-span-8 md:row-span-2 bento-item p-12 flex flex-col justify-between border border-white/5 bg-zinc-950/50 backdrop-blur-xl rounded-none relative overflow-hidden group">
            <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:opacity-10 transition-opacity">
                <span className="material-symbols-outlined text-8xl">auto_awesome</span>
            </div>
            <div className="space-y-2 relative z-10">
              <p className="text-[10px] font-medium tracking-wider text-white/20">Current progress</p>
              <h3 className="text-5xl md:text-6xl font-manrope font-bold tracking-tight leading-none">
                Phase {userSettings?.phase?.split(' ')[1] || "01"}
              </h3>
            </div>

            <div className="mt-24 space-y-6">
              <div className="flex justify-between items-end">
                <p className="text-[10px] font-medium tracking-wider text-white/20">Roadmap</p>
          ...
          <div 
            ...
            className="md:col-span-4 bento-item p-8 tap-scale cursor-pointer border border-white/5 bg-zinc-950/30 hover:bg-zinc-950/50 hover:border-white/20 transition-all rounded-none shadow-lg"
          >
            <div className="flex justify-between items-start mb-12">
              <span className="material-symbols-outlined text-xl opacity-40">play_circle</span>
              <div className={`px-2 py-0.5 border text-[9px] font-medium tracking-wider transition-all duration-300 rounded-none ${
            ...
            {stats.youtube ? (
              <div>
                ...
                <p className="text-[10px] font-medium tracking-wider text-white/20 mt-2 text-zinc-500">YouTube reach</p>
              </div>
            ) : (
          ...
          >
            <div className="flex justify-between items-start mb-12">
              ...
              <div className="px-2 py-0.5 border border-white/10 text-[9px] font-medium tracking-wider text-white/30 rounded-none">
                Spotify
              </div>
            </div>
            ...
            {stats.spotify ? (
              <div className="space-y-1">
                ...
                <p className="text-[10px] font-medium tracking-wider text-white/30 text-zinc-500 mt-2 uppercase">Audience</p>
              </div>
            ) : (
          ...
          {/* Directives (Tasks) */}
          <div className="md:col-span-12 bento-item p-12 pt-16 rounded-none border border-white/5 bg-zinc-950/20">
              ...
              {urgentTasks.length === 0 ? (
                ...
                urgentTasks.map((task, idx) => (
                  <div 
                    key={task.id} 
                    onClick={() => router.push("/tasks")}
                    className={`p-6 flex flex-col justify-between aspect-square tap-scale cursor-pointer rounded-none transition-all ${
                      task.completed ? 'opacity-20 translate-y-2' : 'border border-white/5 bg-zinc-900/50 hover:border-white/20'
                    }`}
                  >
                    <div className="flex justify-between items-start">
                      <p className="text-[10px] font-medium text-white/20">0{idx + 1}</p>
                    ...
                    <p className="text-[10px] font-medium text-white/20 tracking-wider capitalize">{task.phase?.toLowerCase()}</p>
                  </div>
                ))
              )}
            </div>
          ...
          {/* Vault Preview */}
          <div className="md:col-span-12 bento-item p-12 bg-zinc-950/30 tap-scale cursor-pointer border border-white/5 hover:border-white/20 transition-all rounded-none shadow-lg" onClick={() => router.push("/vault")}>
             <div className="flex flex-col md:flex-row gap-12 items-center">
                ...
                <div className="flex-1 w-full grid grid-cols-2 md:grid-cols-5 gap-4 opacity-20">
                    {[...Array(5)].map((_, i) => (
                        <div key={i} className="aspect-square border border-white/5 bg-black rounded-none"></div>
                    ))}
                </div>
             </div>
          </div>
            ...
            <div className="animate-marquee flex gap-20">
                {Array(10).fill(0).map((_, i) => (
                    <p key={i} className="text-[9px] font-medium tracking-widest">
                        OUTLANDIA // ARTIST WORKSPACE // {new Date().getFullYear()} // VERSION 1.0.2
                    </p>
                ))}
            </div>
        </div>
