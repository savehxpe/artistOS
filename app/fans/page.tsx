  ...
  if (loading || !user) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-black font-inter">
        <div className="w-12 h-12 border border-white border-t-zinc-800 animate-spin" />
        <p className="mt-6 text-[10px] font-medium uppercase tracking-[0.4em] text-white/20 animate-pulse">Loading Feed</p>
      </div>
    );
  }
      ...
      <main className="max-w-[1400px] mx-auto px-6 pt-32 pb-32 min-h-screen animate-in fade-in duration-1000 slide-in-from-bottom-5">
        ...
        <section className="mb-16">
            <div className="flex justify-between items-end border-b border-white/10 pb-6 mb-8">
                <h3 className="font-manrope font-black text-sm uppercase tracking-[0.5em] text-white">Insights</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 md:grid-rows-2 gap-4 md:auto-rows-[160px]">
                {/* Conversion Hero */}
                <div className="md:col-span-2 md:row-span-2 bg-zinc-950 p-10 border border-white/10 group hover:border-white transition-all relative overflow-hidden flex flex-col justify-between rounded-none">
                    <div>
                        <p className="font-inter text-[10px] font-black text-white/10 uppercase tracking-[0.4em] mb-2 relative z-10">Conversion</p>
                        <p className="font-inter text-xs text-white/40 font-black uppercase tracking-widest relative z-10">Engagement Efficiency</p>
                    </div>
                ...
                {/* Audience Split */}
                <div className="md:col-span-1 md:row-span-1 bg-zinc-950 p-6 border border-white/10 group hover:border-white transition-all relative overflow-hidden flex flex-col justify-center rounded-none">
                    <p className="font-inter text-[10px] font-black text-white/10 uppercase tracking-[0.4em] mb-4 relative z-10">Audience</p>
                    <div className="space-y-4">
                        <div>
                            ...
                            <div className="w-full h-[1px] bg-white/10"><div className="h-full bg-white/20 w-full"></div></div>
                        </div>
                        <div>
                            <div className="flex justify-between text-[10px] font-black uppercase tracking-widest mb-1">
                            ...
                            <div className="w-full h-[1px] bg-white/10"><div className="h-full bg-white w-[12.4%]"></div></div>
                        </div>
                    </div>
                </div>

                {/* Platform Insights */}
                <div className="md:col-span-1 md:row-span-1 bg-zinc-950 p-6 border border-white/10 group transition-all relative overflow-hidden flex flex-col justify-between rounded-none">
                    <p className="font-inter text-[10px] font-black text-white/10 uppercase tracking-[0.4em] mb-4 relative z-10">Velocity</p>
                    <div className="grid grid-cols-4 gap-2 h-full">
                        {['TK', 'IG', 'YT', 'SP'].map((plat) => (
                            <button key={plat} className="bg-black border border-white/5 flex items-center justify-center font-black text-xs hover:border-white hover:text-white transition-all text-white/10 rounded-none">
                                {plat}
                            </button>
                        ))}
                    </div>
        ...
        {/* SMS Blast Module */}
        <section className="mb-16 bg-zinc-950 border border-white/10 p-10 md:p-16 relative overflow-hidden group rounded-none">
            <div className="relative z-10">
                <div className="flex items-center gap-4 mb-10">
                    <div className="w-10 h-10 bg-white text-black flex items-center justify-center font-bold text-xs rounded-none">SMS</div>
                    <h3 className="font-manrope text-2xl font-bold uppercase tracking-tighter">Direct Broadcast</h3>
                </div>
                
                <div className="space-y-8">
                    <div className="space-y-4">
                        <label className="text-[10px] font-black text-white/10 uppercase tracking-[0.4em]">Message</label>
                        <textarea 
                            value={smsMessage}
                            onChange={(e) => setSmsMessage(e.target.value)}
                            placeholder="Type message..."
                            className="w-full bg-black border border-white/10 p-8 text-lg font-inter focus:outline-none focus:border-white min-h-[160px] resize-none leading-relaxed transition-all text-white placeholder:text-white/5 rounded-none"
                        />
                    </div>
                </div>

                {transmissionResult && (
                    <div className="mt-10 bg-black border border-white/20 p-8 animate-in slide-in-from-top-4 duration-500 rounded-none">
                ...
                <div className="flex flex-col md:flex-row gap-6 mt-10 md:mt-12 items-center justify-between">
                    ...
                    <button 
                        ...
                        className={`w-full md:w-auto px-12 py-5 font-bold text-[11px] uppercase tracking-[0.6em] transition-all active:scale-95 flex items-center justify-center gap-4 rounded-none ${
        ...
        <section className="space-y-10">
          <div className="flex justify-between items-end border-b border-white/10 pb-6">
            <h3 className="font-manrope font-black text-sm uppercase tracking-[0.5em] text-white">Live Feed</h3>
            <div className="flex gap-4 items-center">
          ...
          <div className="space-y-8">
            {feed.map((item) => (
              <div key={item.id} className="group bg-zinc-950 transition-all border-2 border-white/5 hover:border-white/20 overflow-hidden rounded-none">
                <div className="flex items-start gap-8 p-10 border-b border-white/5 group-hover:border-white/10 transition-colors">
                  <div className="relative flex-none">
                    <img src={item.avatar} alt={item.username} className="w-20 h-20 object-cover grayscale group-hover:grayscale-0 transition-all duration-700 border border-white/10 rounded-none" />
                    {item.username.includes("VIP") && <div className="absolute -top-2 -right-2 w-8 h-8 bg-white text-black flex items-center justify-center border border-black text-[10px] font-black shadow-xl rounded-none">VIP</div>}
                  </div>
                ...
                {item.aiSuggestion && (
                  <div className={`p-10 flex items-center justify-between transition-all cursor-pointer group/ai relative ${item.id === "1" ? "bg-white text-black opacity-80 hover:opacity-100 rounded-b-xl" : "bg-zinc-800 text-white rounded-b-xl"}`}>
                    <div className="flex items-center gap-6">
                      <span className="material-symbols-outlined text-4xl">creative_commons</span>
                      <div>
                        <p className="text-[9px] font-black uppercase tracking-[0.5em] mb-2 opacity-20">Creative Insight</p>
                        <span className="font-inter text-sm font-black uppercase tracking-widest leading-tight">"{item.aiSuggestion}"</span>
                      </div>
                    </div>
                    <div className={`w-14 h-14 flex items-center justify-center border group-hover/ai:scale-110 transition-transform rounded-none ${item.id === "1" ? "border-black" : "border-white"}`}>
        ...
        <section className="mt-24 bg-zinc-950 p-12 border border-white/10 relative overflow-hidden group shadow-2xl rounded-none">
          <div className="flex justify-between items-baseline mb-10 relative z-10">
            <h2 className="font-manrope font-black text-3xl tracking-tighter text-white uppercase">Loyalty Index</h2>
          ...
          <div className="w-full h-[2px] bg-white/10 overflow-hidden relative z-10">
