  ...
  return (
    <nav className="fixed bottom-0 w-full z-50 bg-black/90 backdrop-blur-3xl border-t border-white/10 h-24 flex items-center">
      <div className="w-full max-w-[1400px] mx-auto px-6 flex justify-between items-center h-full">
        {/* Global Navigation - Standardized 7-Item Bottom Bar */}
          ...
          {navItems.map((item) => {
            ...
            return (
              <button 
                ...
                className={`flex flex-col md:flex-row items-center gap-2 md:gap-4 px-4 md:px-8 py-3 transition-all group relative rounded-full ${isActive ? "text-black bg-[#FFFF00]" : "text-white/40 hover:text-white"}`}
              >
                <span 
        ...
        <div className="hidden md:flex items-center gap-8 ml-8 border-l border-white/10 pl-8">
            <button 
                ...
                className="group flex items-center gap-3 transition-all active:scale-95 px-5 py-2.5 bg-zinc-900 border border-white/5 hover:border-[#FFFF00]/50 rounded-full"
            >
                <span className="material-symbols-outlined text-white/40 group-hover:text-[#FFFF00] transition-colors text-lg">logout</span>
                <span className="text-[9px] font-bold text-white/40 group-hover:text-[#FFFF00] transition-colors tracking-widest uppercase">Logout</span>
            </button>
        </div>
      </div>
    </nav>
